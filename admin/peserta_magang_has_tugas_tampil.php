                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($permissions['update'] == 1) {
                            echo "<td><a href='index.php?page=peserta_magang_has_tugas_tambah&id_has_tugas=" . $row['id_has_tugas'] . "'>EDIT</a></td>";
                        }

                        // Tombol hapus jika user memiliki izin delete
                        if ($permissions['delete'] == 1) {
                            echo "<td><a href='#' onclick=\"if (confirm('Apakah anda yakin data dihapus ?')) {window.location.href='peserta_magang_has_tugas_proses.php?status=hapus&id_has_tugas=$row[id_has_tugas]';}\">HAPUS</a></td>";
                        }
