...
$roleArray = isset($_SESSION['role']) ? $_SESSION['role'] : [];
$role = isset($roleArray[0]) ? $roleArray[0] : null;
$id_anggota = isset($_SESSION['id_anggota']) ? $_SESSION['id_anggota'] : null; // Ambil id_anggota dari session
...
if ($_SESSION['role'] === 'Siswa') {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($permissions['update'] == 1) {
            echo "<td><a href='index.php?page=peserta_magang_edit&id_anggota={$row['id_anggota']}'>Edit</a></td>";
        }
        if ($permissions['delete'] == 1) {
            echo "<td><a href='index.php?page=peserta_magang_hapus&id_anggota={$row['id_anggota']}' onclick=\"return confirm('Apakah Anda yakin ingin menghapus data ini?')\">Hapus</a></td>";
        }
