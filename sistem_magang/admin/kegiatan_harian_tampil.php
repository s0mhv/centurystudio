...
$id_anggota = $_SESSION['id_anggota'] ?? null; // Ambil id_anggota dari session
...
$id_page = 6; // Misalkan id_page untuk halaman kegiatan_harian tampil adalah 6 (sesuaikan dengan data tabel page)
...
<body>
    <h3>DATA Kegiatan Harian</h3>
    <div class="container">
        <div class="button-container">
            <?php if ($permissions['create'] == 1) { ?>
                <a href='index.php?page=kegiatan_harian_tambah'><input type='submit' name='input' value='TAMBAH DATA'></a>
            <?php } ?>
        </div>
        <div class="table-container">
            <table>
                <tr>
                    <th width="5%">NO.</th>
                    <th width="5%">NAMA</th>
                    <th width="5%">KEGIATAN HARIAN</th>
                    <th width="5%">DESKRIPSI</th>
                    <th width="5%">TANGGAL PENGERJAAN</th>
                    <th width="5%">STATUS KEGIATAN</th>
...
$no = 1;
if ($_SESSION['role'] === 'Siswa') {
    $query = "SELECT kegiatan_harian.*, peserta_magang.nama 
              FROM kegiatan_harian 
              JOIN peserta_magang ON kegiatan_harian.id_anggota = peserta_magang.id_anggota 
              WHERE peserta_magang.id_proposal = ? 
              ORDER BY kegiatan_harian.id_kegiatan_harian";
    $stmt = $koneksi->prepare($query);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($permissions['update'] == 1) {
            echo "<td><a href='index.php?page=kegiatan_harian_tambah&id_kegiatan_harian=" . $row['id_kegiatan_harian'] . "'>EDIT</a></td>";
        }

        if ($permissions['delete'] == 1) {
            echo "<td><a href='index.php?page=kegiatan_harian_proses&id_kegiatan_harian=" . $row['id_kegiatan_harian'] . "' onclick='return confirm(\"Yakin ingin menghapus data ini?\")'>HAPUS</a></td>";
        }
